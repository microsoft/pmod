...
#define _pal_AtomicExchange(destination, exchange) \
...
#define __PAL_ATOMIC_INLINE__ static __inline__ __attribute__((always_inline))

__PAL_ATOMIC_INLINE__ int
_pal_AtomicExchange(int _new, volatile int *ptr)
{
	int prev;
	do {
		prev = *ptr;
...
EXTERN_C PAL_API _pal_AtomicIntegerType _pal_AtomicExchange(
    volatile _pal_AtomicIntegerType *destination,
    _pal_AtomicIntegerType exchange);
